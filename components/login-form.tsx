...
export function LoginForm() {
  ...
  // Check for demo mode parameter and auto-fill credentials
  useEffect(() => {
    const demo = searchParams?.get('demo')
    if (demo === 'true') {
      setIsDemoMode(true)
      setEmail("demo@businessai.com")
      setPassword("demo123")
      toast({
        title: "Demo Mode",
        description: "Demo credentials have been pre-filled. Click 'Sign In' to continue with sample data.",
        duration: 5000,
      })
    }
  }, [searchParams, toast])
      ...
      <Card className="shadow-2xl border border-border bg-card/80 backdrop-blur-sm">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription className="text-muted-foreground">
            {isDemoMode ? "Demo Mode - Sample Data Experience" : "Enterprise Decision Intelligence Platform"}
          </CardDescription>
          {isDemoMode && (
            <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mt-4">
              <p className="text-sm text-blue-800 dark:text-blue-200">
                🎯 Demo credentials pre-filled! Experience all features with sample data.
              </p>
            </div>
          )}
        </CardHeader>
            ...
            >
              {isLoading ? (
                <div className="flex items-center gap-2">
                  ...
                  {isDemoMode ? "Accessing Demo..." : "Signing In..."}
                </div>
              ) : (
                isDemoMode ? "Access Demo" : "Sign In"
              )}
            </Button>
